...
from FAENAS_BARCOS
where TO_CHAR(FechaInicio, 'YYYY') = ('2013')
...
select MatriculaBarco
from FAENAS_BARCOS
where TO_CHAR(FechaInicio, 'YYYY') = ('2014')
AND ReferenciaCaladero IN (select Referencia
                           from CALADEROS
                           where Nombre = 'Gran Sol')
INTERSECT
select MatriculaBarco
from FAENAS_BARCOS
where TO_CHAR(FechaInicio, 'YYYY') = ('2015')
AND ReferenciaCaladero IN (select Referencia
                           from CALADEROS
                           where Nombre = 'Gran Sol');
...
as
select MatriculaBarco, ReferenciaCaladero, sum(TRUNC(FechaFin) - TRUNC(FechaInicio)) as DiasDePesca
from FAENAS_BARCOS
...
create or replace view SelectFinal
as
