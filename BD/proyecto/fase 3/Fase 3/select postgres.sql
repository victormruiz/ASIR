...
AND cd.FechaPescaFin > (current_date - interval '6 months')
group by DNIArmador;
...
from CAPTURAS_DIARIAS
where FechaPescaFin > (current_date - interval '12 months')
...
from FAENAS_BARCOS
where extract(year from FechaInicio)  IN ('2013','2014','2015')
...
AND ProximoPago > (current_date - interval '19 months')
...
from FAENAS_BARCOS
where fechaInicio > (current_date - interval '24 months');
...
as
select distinct MatriculaBarco, months_between(current_date, ProximoPago) as mesesporpagar, CodigoPuntoDeAtraque
from PAGOS_ATRAQUES
where ProximoPago < current_date
