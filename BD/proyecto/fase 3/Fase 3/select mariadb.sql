...
select MatriculaBarco, ReferenciaCaladero,date_add(sysdate, interval 15 day)
from BarcoPunto, CaladeroFinal, dual;
...
AND cd.FechaPescaFin > date_add(SYSDATE, interval -6 month)
group by DNIArmador;
...
where DNI = (select DNIArmador
            from Final
            where DNIArmador = f.DNIArmador);
...
from CAPTURAS_DIARIAS
where FechaPescaFin > date_add(sysdate, - interval -12 month)
...
from FAENAS_BARCOS
where STR_TO_DATE(FechaInicio, '%Y') IN ('2013','2014','2015')
...
select Referencia
from CALADEROS
Where Extension > 200000;

create or replace view CodigoDeGambaBlanca
as
select Codigo
from ESPECIES_DE_PESCADOS
where NombreComercial = 'Acedia';
...
as
select distinct MatriculaBarco, 0 as TotalKilos
from CAPTURAS_DIARIAS
where ReferenciaCaladero IN (select Referencia
                             from CaladerosDeMasDe2000)
AND MatriculaBarco NOT IN (Select MatriculaBarco
...
from PAGOS_ATRAQUES
where ProximoPago < sysdate
AND ProximoPago > date_add(sysdate, interval -19 month)
...
as
select MatriculaBarco, ReferenciaCaladero, TRUNC(FechaFin) - TRUNC(FechaInicio) as DiasDePesca
from FAENAS_BARCOS
where fechaInicio > date_add(sysdate, INTERVAL -24 month);
...
as
select distinct MatriculaBarco, months_between(sysdate, ProximoPago) as mesesporpagar, CodigoPuntoDeAtraque
from PAGOS_ATRAQUES
where ProximoPago < sysdate
