...
CONSTRAINT ck_NumeroCuenta CHECK (NumeroCuenta REGEXP '[A-Z]{2}[0-9]{2}\ [0-9]{4}\ [0-9]{4}\ [0-9]{4}\ [0-9]{4}\ [0-9]{4}$'),
CONSTRAINT ck_Ciudad CHECK (Ciudad LIKE '%(España)' OR Ciudad LIKE '%(Portugal)')
);
...
CONSTRAINT ck_Matricula CHECK ((Matricula REGEXP '6ª\-[A-Z]{2}\-[0-9]\-[0-9]{1,3}\-[0-9]{2}$') OR (Matricula REGEXP '7ª\-[A-Z]{2}\-[0-9]{1}\-[0-9]{1,3}\-[0-9]{2}$'))
);
...
CONSTRAINT ck_FechaInicio CHECK (SUBSTR(CAST(FechaInicio AS CHAR),6,5) NOT BETWEEN '04-15' AND '06-15'),
CONSTRAINT ck_FechaFin CHECK (SUBSTR(CAST(FechaFin AS CHAR),6,5) NOT BETWEEN '04-15' AND '06-15')
);
...
Kilos FLOAT(6),
Tamaño FLOAT(6),
FechaPescaInicio DATETIME,
FechaPescaFin DATETIME,
...
CONSTRAINT ck_FechaPescaInicio CHECK (TIME(FechaPescaInicio) >= '05:30:00')
);
